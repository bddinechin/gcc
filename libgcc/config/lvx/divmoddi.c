...
uint64_divmod (uint64_t a, uint64_t b)
{
  ...
  uint64_t q = __builtin_lvx_selectd (a >= b, a, bbig, ".dnez");
  // uint64_t r = bbig ? a - (b&-q) : 0;
  uint64_t r = __builtin_lvx_selectd (a - (b & -q), 0, bbig, ".dnez");
  double doublea = __builtin_lvx_floatud (a, 0,  ".rn.s");
  double doubleb = __builtin_lvx_floatud (b, 0, ".rn.s");
  float floatb = __builtin_lvx_fnarrowdw (doubleb, ".rn.s");
  ...
  q = __builtin_lvx_selectd (quo + cond, q, special, ".deqz");
  // r = !special ? rem + (b & cond) : r;
  r = __builtin_lvx_selectd (rem + (b & cond), r, special, ".deqz");
  return (uint64x2_t){q, r};
...
int64_t
__divmoddi4 (int64_t a, int64_t b, int64_t *c)
{
  uint64_t absa = __builtin_lvx_absd (a, "");
  uint64_t absb = __builtin_lvx_absd (b, "");
