...
uint64x2_divmod (uint64x2_t a, uint64x2_t b)
{
  ...
  uint64x2_t q = __builtin_lvx_selectdp (-(a >= b), a, bbig, ".nez");
  // uint64x2_t r = bbig ? a - (b&-q) : 0;
  uint64x2_t r
    = __builtin_lvx_selectdp (a - (b & -q), 0 - (uint64x2_t){}, bbig, ".nez");
  float64x2_t doublea = __builtin_lvx_floatudp (a, 0, ".rn.s");
  float64x2_t doubleb = __builtin_lvx_floatudp (b, 0, ".rn.s");
  ...
  if (__builtin_lvx_anydp (b, ".eqz"))
    goto div0;
  float64x2_t doublerec = {__builtin_lvx_fwidenwd (floatrec_0, ".s"),
			   __builtin_lvx_fwidenwd (floatrec_1, ".s")};
  float64x2_t doubleq0 = __builtin_lvx_fmuldp (doublea, doublerec, ".rn.s");
  uint64x2_t q0 = __builtin_lvx_fixedudp (doubleq0, 0, ".rn.s");
  int64x2_t a1 = (int64x2_t)(a - q0 * b);
  float64x2_t alpha
    = __builtin_lvx_ffmsdp (doubleb, doublerec, double1, ".rn.s");
  float64x2_t beta
    = __builtin_lvx_ffmadp (alpha, doublerec, doublerec, ".rn.s");
  float64x2_t doublea1 = __builtin_lvx_floatdp (a1, 0, ".rn.s");
  float64x2_t gamma = __builtin_lvx_fmuldp (beta, doublea1, ".rn.s");
  int64x2_t q1 = __builtin_lvx_fixeddp (gamma, 0, ".rn.s");
  int64x2_t rem = a1 - q1 * b;
  uint64x2_t quo = q0 + q1;
  uint64x2_t cond = (uint64x2_t)(rem >> 63);
  // q = !special ? quo + cond : q;
  q = __builtin_lvx_selectdp (quo + cond, q, special, ".eqz");
  // r = !special ? rem + (b & cond) : r;
  r = __builtin_lvx_selectdp (rem + (b & cond), r, special, ".eqz");
...
{
  uint64x2_t absa = __builtin_lvx_absdp (a, "");
  uint64x2_t absb = __builtin_lvx_absdp (b, "");
  uint64x4_t divmod = uint64x2_divmod (absa, absb);
  int64x2_t result = __builtin_lvx_low128 (divmod);
  return __builtin_lvx_selectdp (-result, result, a ^ b, ".ltz");
}
...
__modv2di3 (int64x2_t a, int64x2_t b)
{
  ...
  uint64x2_t absb = __builtin_lvx_absdp (b, "");
  uint64x4_t divmod = uint64x2_divmod (absa, absb);
  int64x2_t result = __builtin_lvx_high128 (divmod);
  return __builtin_lvx_selectdp (-result, result, a, ".ltz");
}
