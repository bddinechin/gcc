...
// word is 0
#define RAISE_INEXACT (__builtin_lvx_wfxl (4, INEXACT_CST << 32))
#define RAISE_UNDERFLOW                                                        \
  (__builtin_lvx_wfxl (4, (UNDERFLOW_CST | INEXACT_CST) << 32)) // underflow always raised when inexact
#define RAISE_OVERFLOW (__builtin_lvx_wfxl (4, OVERFLOW_CST << 32))
#define RAISE_DIVZERO (__builtin_lvx_wfxl (4, DIVZERO_CST << 32))
#define RAISE_INVALID (__builtin_lvx_wfxl (4, INVALID_CST << 32))

#define RESET_EV (__builtin_lvx_wfxl (4, EXCEPTION_MASK))

// returns: Inexact | Underflow | Overflow | Div by zero | Invalid | 0
#define GET_EV ((__builtin_lvx_get (4) & EXCEPTION_MASK))
#define OR_SET_EV(EV) (__builtin_lvx_wfxl (4, EV << 32))

#define GET_ROUND ((__builtin_lvx_get (4) >> 16) & ROUND_MASK)
...
{                                                                             \
  ...
  fp_type r2 = __builtin_lvx_ffms (r_db.d, q_star, r_da.d, ".rn.s");          \
                                                                              \
...
{                                                                             \
  ...
  fp_type q_snd    = __builtin_lvx_ffma (r2, y3, q_star, ""    SILENT_STRING);\
  fp_type q_snd_ru = __builtin_lvx_ffma (r2, y3, q_star, ".ru" SILENT_STRING);\
  fp_type q_snd_rd = __builtin_lvx_ffma (r2, y3, q_star, ".rd" SILENT_STRING);\
                                                                              \
  ...
  fp_type UNUSED (res_r_cs) = res.d =                                         \
    __builtin_lvx_fmul (r_dq.d, scale.d, "" SILENT_STRING);                   \
                                                                              \
  if (!__builtin_strcmp (#mode1, "rn"))                                       \
    {                                                                         \
      ...
      fp_type res_rd =                                                        \
        __builtin_lvx_fmul (r_dq.d, scale.d, ".rd" SILENT_STRING);            \
      fp_type res_ru =                                                        \
        __builtin_lvx_fmul (r_dq.d, scale.d, ".ru" SILENT_STRING);            \
                                                                              \
...
{                                                                             \
  ...
  iterate_approx (r_da, r_db, &y3, &q_star);                                  \
                                                                              \
  fp_type r2 = __builtin_lvx_ffms (r_db.d, q_star, r_da.d,                    \
                                      "." #mode1 ".s");                       \
  /* Not silent, will detect inexact */                                       \
  fp_type q_snd = __builtin_lvx_ffma (r2, y3, q_star,                         \
                                         "." #mode1 SILENT_STRING);           \
                                                                              \
  /* Not silent, will detect inexact */                                       \
  fp_type q_snd_ ## mode2 = __builtin_lvx_ffma (r2, y3, q_star,               \
                                                   "." #mode2 SILENT_STRING); \
                                                                              \
  /* To detect inexact without doubt, compare results in round up and round   \
   * down mode */                                                             \
  int is_inexact = is_inexact_p (                                             \
      __builtin_lvx_ffma (r2, y3, q_star, ".ru.s"),                           \
      __builtin_lvx_ffma (r2, y3, q_star, ".rd.s"));                          \
                                                                              \
  FP_UINT r_dq = {                                                            \
  ...
  fp_type UNUSED (res_rn) = dres.d =                                                   \
    __builtin_lvx_fmul (r_dq.d, scale.d, "." #mode1 SILENT_STRING);           \
                                                                              \
  if (!__builtin_strcmp (#mode1, "rn"))                                       \
    {                                                                         \
      /* Step 7: treat midpoint subnormal case, only in round to nearest      \
...
{                                                                             \
  if (!__builtin_strcmp (PROTECT(fp_type), "double"))                         \
    {                                                                         \
      /* Step 2: get seed for 1/b */                                          \
      FP_UINT y0 = {.d = __builtin_lvx_fsrecd (r_db.d, ".s")};                \
      /* For debug: */                                                        \
      /* if ((y0.u & 1) != 1)                                                 \
      ...
      fp_type e0 = __builtin_lvx_ffmsd (r_db.d, y0.d, 1.0, ".rn.s");          \
      fp_type y1 = __builtin_lvx_ffmad (y0.d, e0, y0.d, ".rn.s");             \
      fp_type e1 = __builtin_lvx_fmuld (e0, e0, ".rn.s");                     \
      fp_type y2 = __builtin_lvx_ffmad (y1, e1, y1, ".rn.s");                 \
                                                                              \
      fp_type r = __builtin_lvx_ffms (r_db.d, y2, 1.0, ".rn.s");              \
    ...
    {                                                                         \
      ...
      *y3 = __builtin_lvx_frecw (r_db.d, ".rn.s");                            \
    }                                                                         \
                                                                              \
  /* Step 4: 2 markstein on a/b for correct result */                         \
  /* q = approx on a/b */                                                     \
  fp_type q = __builtin_lvx_fmul (r_da.d, *y3, ".rn.s");                      \
  fp_type r1 = __builtin_lvx_ffms (r_db.d, q, r_da.d, ".rn.s");               \
  *q_star = __builtin_lvx_ffma (r1, *y3, q, ".rn.s");                         \
}                                                                             \
