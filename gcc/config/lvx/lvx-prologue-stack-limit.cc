...
unsigned int
lvx_prologue_stack_limit (function *fun)
{
  basic_block prologue = single_succ (ENTRY_BLOCK_PTR_FOR_FN (fun));
  rtx (*gen_cbranch) (rtx, rtx, rtx, rtx) = gen_cbranchdi4;

  int size = lvx_get_real_frame_size (fun);
...
public:
  ...
  {
    return lvx_prologue_stack_limit (fun);
  }
