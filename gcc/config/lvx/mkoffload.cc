/* Offload image generation tool for LVX.
...
/* Munges LVX assembly into a C source file defining the LVX code as a
   string.
...
static bool lvx_mkoffload_debug;

/* Print a message to stderr if LVX_DEBUG value is set to true.  */

#define DEBUG_PRINT(...) \
  do \
  { \
    if (lvx_mkoffload_debug) \
      { \
...
/* Flush stderr if LVX_DEBUG value is set to true.  */
...
/* Print a logging message with PREFIX to stderr if LVX_DEBUG value
   is set to true.  */
...
#define LVX_DEBUG(...) DEBUG_LOG ("LVX debug: ", __VA_ARGS__)

/* Print a warning message to stderr.  */

#define LVX_WARNING(...) DEBUG_LOG ("LVX warning: ", __VA_ARGS__)
...
const char tool_name[] = "lvx-mkoffload";

/* Files to unlink.  */
static const char *lvx_s1_name;
static const char *lvx_s2_name;
static const char *lvx_o_name;
static const char *lvx_cfile_name;
...
{
  if (lvx_cfile_name)
    maybe_unlink (lvx_cfile_name);
  if (lvx_s1_name)
    maybe_unlink (lvx_s1_name);
  if (lvx_s2_name)
    maybe_unlink (lvx_s2_name);
  if (lvx_o_name)
    maybe_unlink (lvx_o_name);
}
...
process_asm (FILE * in, FILE * out, FILE * cfile)
{
  ...
  /* Support for omp_is_initial_device.
     omp_is_initial_device performs its detection by trying to locate the symbol .__omp.lvx.target_p;
   */
  ...
  fprintf (cfile, "static const struct lvx_kernel_description {\n"
	   "  const char *name;\n" "} lvx_kernels[] = {\n  ");
...
process_obj (FILE * in, FILE * cfile, uint32_t omp_requires)
{
  ...
  fprintf (cfile, "static unsigned char lvx_code[] = {");
  ...
  fprintf (cfile,
	   "static const struct lvx_image {\n"
	   "  size_t size;\n"
	   "  void *image;\n"
	   "} lvx_image = {\n" "  %zu,\n" "  lvx_code\n" "};\n\n", len);

  fprintf (cfile,
	   "static const struct lvx_image_desc {\n"
	   "  uintptr_t omp_requires_mask;\n"
	   "  const struct lvx_image *lvx_image;\n"
	   "  unsigned kernel_count;\n"
	   "  const struct lvx_kernel_description *kernel_infos;\n"
	   "  unsigned global_variable_count;\n"
	   "  const struct global_var_info *global_variables;\n"
	   "} target_data = {\n"
	   " %d,\n"
	   "  &lvx_image,\n"
	   "  sizeof (lvx_kernels) / sizeof (lvx_kernels[0]),\n"
	   "  lvx_kernels,\n"
  ...
  fprintf (cfile, "static __attribute__((constructor)) void init (void)\n"
	   ...
	   " %d/*LVX*/, &target_data);\n"
	   "};\n", GOMP_VERSION_PACK (GOMP_VERSION, GOMP_VERSION /*_LVX*/ ),
	   GOMP_DEVICE_KALRAY_LVX);
...
main (int argc, char **argv)
{
  ...
  lvx_s1_name = make_temp_file (".mkoffload.1.s");
  lvx_s2_name = make_temp_file (".mkoffload.2.s");
  lvx_o_name = make_temp_file (".mkoffload.lvxco");
  lvx_cfile_name = make_temp_file (".c");

  if (secure_getenv ("MKOFFLOAD_DEBUG"))
    lvx_mkoffload_debug = true;
  else
    lvx_mkoffload_debug = false;
...
//    obstack_ptr_grow (&cc_argv_obstack, "-mgomp");
  ...
  obstack_ptr_grow (&cc_argv_obstack, lvx_s1_name);
...
//  obstack_ptr_grow (&ld_argv_obstack, "-c");
  obstack_ptr_grow (&ld_argv_obstack, lvx_s2_name);
//  obstack_ptr_grow (&ld_argv_obstack, "-lgomp");
  ...
  obstack_ptr_grow (&ld_argv_obstack, lvx_o_name);
  ...
  cfile = fopen (lvx_cfile_name, "w");
  if (!cfile)
    fatal_error (input_location, "cannot open '%s'", lvx_cfile_name);
  ...
  if (offload_abi == OFFLOAD_ABI_LP64)
	...
	omp_requires_file = make_temp_file (".mkoffload.omp_requires");
      ...
      LVX_DEBUG ("dumppfx: %s, omp_requires_file: %s\n", dumppfx, omp_requires_file);
	...
	fatal_error (input_location, "cannot read omp_requires file %qs",
      ...
      in = fopen (lvx_s1_name, "r");
      if (!in)
	fatal_error (input_location, "cannot open intermediate lvx asm file");

      out = fopen (lvx_s2_name, "w");
      if (!out)
	fatal_error (input_location, "cannot open '%s'", lvx_s2_name);
      ...
      in = fopen (lvx_o_name, "r");
      if (!in)
	fatal_error (input_location, "cannot open intermediate lvx obj file");
  ...
  compile_native (lvx_cfile_name, outname, collect_gcc);
