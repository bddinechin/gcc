...
;; COMP* and COMPN* do NOT produce the same shape of result, so only one of
;; them replaces a pair.
;;
...
;;                       cannot implement (neg (cmp)) -- the 0/1-per-lane vector
;;                       -- and a pattern that tries emits "compdp $r0r1 = ..."
;;                       which the assembler rejects.
;;
;; So (neg (cmp)) stays a pair of *scalar* COMPD/FCOMPD over the halves, each
...
;;
;; Vector modes exist only on lvx-2 (lvx_vector_mode_supported_p), so the LVX_2
;; gate on the COMPN* patterns costs nothing on lvx-1.
...
;; The FP compares split exactly as the integer ones above: FCOMPN{HO,WQ,DP}
;; writes the full 0/-1 mask into a pair and replaces a pair of scalar FCOMPD,
;; while FCOMP{HO,WQ,DP} writes a packed bit per lane into a single register and
;; therefore cannot implement (neg (cmp)), which stays a scalar FCOMPD pair.
