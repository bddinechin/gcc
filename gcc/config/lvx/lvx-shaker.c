/* Copyright (C) 1991-2022 Free Software Foundation, Inc.

   This file is part of GCC.

   GCC is free software; you can redistribute it and/or modify it
   under the terms of the GNU General Public License as published
   by the Free Software Foundation; either version 3, or (at your
   option) any later version.

   GCC is distributed in the hope that it will be useful, but WITHOUT
   ANY WARRANTY; without even the implied warranty of MERCHANTABILITY
   or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public
   License for more details.

   You should have received a copy of the GNU General Public License
   along with GCC; see the file COPYING3.  If not see
   <http://www.gnu.org/licenses/>.  */

#define IN_TARGET_CODE 1

#include "config.h"
#include "errors.h"
#include "system.h"
#include "coretypes.h"
#include "opts.h"
#include "tree-pass.h"
#include "backend.h"
#include "memmodel.h"
#include "basic-block.h"
#include "rtl.h"
#include "df.h"
#include "tree.h"
#include "expr.h"
#include "cfghooks.h"
#include "insn-codes.h"
#include "optabs.h"
#include "emit-rtl.h"
#include "print-rtl.h"
#include "cfgrtl.h"
#include "output.h"
#include "recog.h"
#include "insn-attr.h"
#include "rtl.h"
#include "pretty-print.h"
#include "genrtl.h"
#include "insn-flags.h"

#include "lvx-protos.h"

/* The shaker is initialized only once and remember its state */
enum lvx_shaker_state
{
  /* This is the state if the pass has not be called yet. */
  LVX_SHAKER_UNINIT,
  /* At the first invocation of the pass, if the switch to turn on the
   * pass is not present the shaker is disabled */
  LVX_SHAKER_DISABLED,
  /* Otherwise it is enabled and initialized */
  LVX_SHAKER_ENABLED
};
static enum lvx_shaker_state lvx_shaker_state;
static int lvx_shaker_seed;

/* Structure which store the basic information about loads/stores. */
struct GTY (()) lvx_shaker_ls
{
  rtx src;
  rtx dest;
};

static int
shaker_rand ()
{
  lvx_shaker_seed = (lvx_shaker_seed * 1103515245U + 12345U) & 0x7fffffffU;
  return lvx_shaker_seed;
}

static int
load_p (rtx_insn * insn, struct lvx_shaker_ls *ls_info)
{
  if (GET_CODE (PATTERN (insn)) != SET)
    return 0;

  enum attr_type insn_type = get_attr_type (insn);
  if (!(insn_type >= TYPE_LOAD_CORE && insn_type < TYPE_STORE_CORE))
    return 0;

  if (ls_info)
    {
      rtx src = SET_SRC (PATTERN (insn));
      rtx dest = SET_DEST (PATTERN (insn));
      if (GET_CODE (src) == UNSPEC || GET_CODE (XEXP (src, 0)) == MEM)
	return 0;
      if (GET_CODE (XEXP (src, 0)) == PLUS &&
	  (GET_CODE (XEXP (XEXP (src, 0), 0)) == MULT ||
	   GET_CODE (XEXP (XEXP (src, 0), 1)) == MULT))
	return 0;
      ls_info->src = copy_rtx (XEXP (src, 0));
      ls_info->dest = copy_rtx (dest);
    }

  return 1;
}

/* Shake offsets of the form PLUS(REG, CONST_INT). */
static inline void
shake_offset (rtx src)
{
  if (GET_CODE (src) != PLUS)
    return;

  switch (GET_CODE (XEXP (src, 1)))
    {
    case CONST_INT:
      {
	int offset = INTVAL (XEXP (src, 1));
	offset += ((shaker_rand () & 1) ? 1 : -1) * 64 * (shaker_rand () % 8);
	XEXP (src, 1) = GEN_INT (offset);
      }
    default:
      break;

    }
}

/* Main entry point for this pass.  */
static unsigned int
lvx_shaker (function * fun)
{
  basic_block bb;
  FOR_EACH_BB_FN (bb, fun)
  {
    for (rtx_insn * cur_insn = BB_HEAD (bb); cur_insn != BB_END (bb);
	 cur_insn = NEXT_INSN (cur_insn))
      {
	int shake_p = shaker_rand ();
	if (shake_p % 5)
	  continue;
	switch (GET_CODE (cur_insn))
	  {
	  case INSN:
	    {
	      struct lvx_shaker_ls ls_info = { 0, 0 };
	      int is_load = load_p (cur_insn, &ls_info);
	      if (shake_p % 51 == 0)
		emit_insn_before (gen_nop (), cur_insn);
	    }
	  default:
	    break;
	  }
      }
    insn_locations_finalize ();
    init_insn_lengths ();
  }

  return 0;
}

const pass_data pass_data_lvx_shaker = {
  RTL_PASS,			/* type */
  "lvx_shaker",			/* name */
  OPTGROUP_NONE,		/* optinfo_flags */
  TV_NONE,			/* tv_id */
  0,				/* properties_required */
  0,				/* properties_provided */
  0,				/* properties_destroyed */
  0,				/* todo_flags_start */
  0,				/* todo_flags_finish */
};

class pass_lvx_shaker:public rtl_opt_pass
{
public:
  pass_lvx_shaker (gcc::context * ctxt):rtl_opt_pass (pass_data_lvx_shaker,
						      ctxt)
  {
  }

  /* opt_pass methods: */
  virtual bool gate (function *)
  {
    if (lvx_shaker_state == LVX_SHAKER_UNINIT)
      {
	int i;
	cl_deferred_option *opt;
	vec < cl_deferred_option > *v
	  = (vec < cl_deferred_option > *)lvx_deferred_options;

	if (v)
	  FOR_EACH_VEC_ELT (*v, i, opt)
	  {
	    switch (opt->opt_index)
	      {
	      case OPT_fshaker_seed_:
		lvx_shaker_state = LVX_SHAKER_ENABLED;
		lvx_shaker_seed = atoi (opt->arg);
		break;
	      default:
		break;
	      }
	    if (!lvx_shaker_state)
	      lvx_shaker_state = LVX_SHAKER_DISABLED;
	  }
      }

    return lvx_shaker_state == LVX_SHAKER_ENABLED;
  }

  virtual unsigned int execute (function * fun)
  {
    return lvx_shaker (fun);
  }

  opt_pass *clone ()
  {
    return new pass_lvx_shaker (m_ctxt);
  }

};				// class pass_lvx_shaker

rtl_opt_pass *
make_pass_lvx_shaker (gcc::context * ctxt)
{
  return new pass_lvx_shaker (ctxt);
}
